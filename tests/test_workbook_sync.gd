...
func _init() -> void:
	...
	_expect(story.row_count() == 147, "同步结果应包含序章 + v6 C01—C22 共 147 条活动剧情")
	...
	_expect(_row_field_contains(story, "P0001B", "NPC台词", "跑了二十年。人也是在那儿没的。") and _row_field_contains(story, "P0001B", "NPC台词", "材料给我看看。这趟跑完，明天还让我接着跑吗？"), "S00 没有同步死亡地点钩子、查看材料与稿位追问")
	_expect(_row_field_contains(story, "P0001C", "NPC台词", "十一点半截稿。明天的事，等稿子回来再说。") and _row_field_equals(story, "P0001C", "下一话题", "前序·自由探索"), "S00 领取材料没有保留截稿压力与未承诺下一篇稿位")
